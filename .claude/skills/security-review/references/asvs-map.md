...
| 3.4.1 | `Strict-Transport-Security` on all responses, `max-age ≥ 1yr` (+subdomains for L2)     | 1   | met     | `Strict-Transport-Security "max-age=63072000; includeSubDomains; preload"` (`Caddyfile`, security.md rule 7)                                                                                                                                        | code review (`Caddyfile`)      |
...
| V12 Secure Communication | partial (edge)     | TLS is terminated by Caddy (TLS 1.2/1.3, HSTS + preload); the SPA serves only same-origin relative paths (`Caddyfile`, `decisions.md` #5/#10).                                                     |
